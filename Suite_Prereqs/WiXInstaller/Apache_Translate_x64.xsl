...
<xsl:stylesheet version="1.0"
  ...
  <xsl:key name="httpdexe-element-id" match="wix:File[substring(@Source,string-length(@Source) - string-length('httpd.exe') +1) ='httpd.exe' ]" use="@Id" />
  ...
  <xsl:key name="httpdexe-element-component-id" match="wix:File[substring(@Source, string-length(@Source) - string-length('httpd.exe') +1) = 'httpd.exe' ]" use="parent::*/@Id" />
  <!-- Remove the file element -->
  <xsl:template match="wix:File[key('httpdexe-element-id', @Id)]" />
  <!-- Remove the parent component element -->
  <xsl:template match="wix:Component[key('httpdexe-element-component-id', @Id)]" />
  <!-- Remove ComponentRefs referencing of the component(s). -->
  <xsl:template match="wix:ComponentRef[key('httpdexe-element-component-id', @Id)]" />
