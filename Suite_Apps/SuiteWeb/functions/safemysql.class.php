...
class SafeMySQL
{
	...
	{
        global $db_user, $db_pass, $db_db, $db_server, $db_port;

        $this->defaults['host']=$db_server;
        $this->defaults['port']=$db_port;
        $this->defaults['user']=$db_user;
        $this->defaults['pass']=$db_pass;
        $this->defaults['db']=$db_db;
	 ...
	 * @return true|false
	 */
    public function autocommit($tf)
    {
        if(mysqli_autocommit($this->conn, $tf))
        {
            return true;
        }
        else
	...
	/**
	 * Calls mysqli_data_seek
	 *
	 * Examples:
	 * $db->rollback();
	 *
	 * @param resource $result - myqli result
	 * @param int $offset - pointer to seek
	 * @return true|false
	 */
	 ...
	 *
	 * Examples:
	 * $db->setErrorMode('error' | 'exception');
	 *
	 * @param string $emode - 'error'|'exception'
	 * @return true|false
	 */
    public function setErrorMode($emode)
    {
        if($emode=='error' || $emode=='exception')
        {
            $this->emode=$emode;
            return true;
        }
    ...
    /**
	 * Conventional function to run a multiple query with placeholders. A mysqli_multi_query wrapper with placeholders support
	 *
	 * Examples:
	 * $db->multi_query("DELETE FROM table WHERE id=?i", $id);
	 *
	...
	{
		return $this->rawMulti_query($this->prepareQuery(func_get_args()));
	}
	 ...
	 */
	private function rawMulti_query($query)
	{
	 ...
	 *
	 * @param resource $result - myqli result
	 * @return object whatever mysqli_fetch_field returns
	 */
	 ...
	 * @return object whatever mysqli_fetch_field_direct returns
	 */
	public function fetch_field_direct($result,$fieldnr)
	{
		return mysqli_fetch_field_direct($result,$fieldnr);
	}
	 ...
	 * 
	 * @param resource $result - myqli result
	 * @return int whatever mysqli_num_fields returns
	 */
	...
	private function error($err)
	{
        ...
        LogError($err);
        LogError(__CLASS__ . " Stats: " . print_r($this->getStats(),true));
