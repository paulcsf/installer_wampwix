...
// slimbyte update - LaunchAfterInstall feature - execute after install process on close button click
static const LPCWSTR WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_TARGET_PATH = L"LaunchAfterInstallTarget";
static const LPCWSTR WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_ARGUMENTS = L"LaunchAfterInstallArguments";
static const LPCWSTR WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_HIDDEN = L"LaunchAfterInstallHidden";
static const LPCWSTR WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_WORK_FOLDER = L"LaunchAfterInstallWorkingFolder";


// slimbyte update - InstallationLogsFolder - a defined paths where bundle should place installation logs on completed
static const LPCWSTR WIXSTDBA_VARIABLE_PHYSICALMEMORY = L"PhysicalMemory";
...
static const LPCWSTR WIXSTDBA_VARIABLE_BUNDLE_TEMP_PATH = L"WixBundleTempPath";
static const LPCWSTR WIXSTDBA_VARIABLE_LANGUAGE_ID = L"WixStdBALanguageId";

static const LPCWSTR WIXSTDBA_VARIABLE_XNDIAG_PREREQS = L"XNDiagPrereqs";
static const LPCWSTR WIXSTDBA_VARIABLE_DONOTCLEANUPDATADIR = L"DoNotCleanUpDataDir";
static const LPCWSTR WIXSTDBA_VARIABLE_SUITEVERSIONFOUND = L"SuiteVersionFound";




static const LPCWSTR MSG_DATADIR_EXISTS = L"The Install location already contain a non empty \"MySQLData\" directory from a previous installation. Setup can use existing data directory or can drop it and initialize a new one.\nIf existing data directory is used, the database root password, specified in Install Options, must match with the one from previous installation.\n\nDo you want to use extisting database directory?\n\tClick Yes to keep and use extisting database directory.\n\tClick No to remove existing database directory.\n\tClick Cancel to cancel the installation.";




static const HRESULT E_WIXEXTBA_ANOTHER_INSTANCE_RUNNING = MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIX, 1001);
...
enum WIXSTDBA_CONTROL
{
	...
	// Progress page
	// slimbyte update - Install / Repair / Uninstall progress header and label
	WIXSTDBA_CONTROL_PROGRESS_HEADER,
	WIXSTDBA_CONTROL_PROGRESS_INSTALL_HEADER, 
	WIXSTDBA_CONTROL_PROGRESS_UNINSTALL_HEADER,
	WIXSTDBA_CONTROL_PROGRESS_REPAIR_HEADER, 
	WIXSTDBA_CONTROL_PROGRESS_LABEL,
	WIXSTDBA_CONTROL_PROGRESS_INSTALL_LABEL,
	WIXSTDBA_CONTROL_PROGRESS_UNINSTALL_LABEL,
	WIXSTDBA_CONTROL_PROGRESS_REPAIR_LABEL,
	...
	// slimbyte update - Install / Repair / Uninstall success label
	WIXSTDBA_CONTROL_SUCCESS_LABEL, 
	WIXSTDBA_CONTROL_SUCCESS_INSTALL_LABEL, 
	WIXSTDBA_CONTROL_SUCCESS_UNINSTALL_LABEL,
	WIXSTDBA_CONTROL_SUCCESS_REPAIR_LABEL, 
...
static THEME_ASSIGN_CONTROL_ID vrgInitControls[] = {
	...
	// slimbyte update - Install / Repair / Uninstall progress header and label
	{ WIXSTDBA_CONTROL_PROGRESS_HEADER, L"ProgressHeader" }, 
	{ WIXSTDBA_CONTROL_PROGRESS_INSTALL_HEADER, L"ProgressInstallHeader" }, 
	{ WIXSTDBA_CONTROL_PROGRESS_UNINSTALL_HEADER, L"ProgressUninstallHeader" },
	{ WIXSTDBA_CONTROL_PROGRESS_REPAIR_HEADER, L"ProgressRepairHeader" }, 
	{ WIXSTDBA_CONTROL_PROGRESS_LABEL, L"ProgressLabel" }, 
	{ WIXSTDBA_CONTROL_PROGRESS_INSTALL_LABEL, L"ProgressInstallLabel" }, 
	{ WIXSTDBA_CONTROL_PROGRESS_UNINSTALL_LABEL, L"ProgressUninstallLabel" },
	{ WIXSTDBA_CONTROL_PROGRESS_REPAIR_LABEL, L"ProgressRepairLabel" }, 
	...
	// slimbyte update - Install / Repair / Uninstall success label
	{ WIXSTDBA_CONTROL_SUCCESS_LABEL, L"SuccessLabel" }, 
	{ WIXSTDBA_CONTROL_SUCCESS_INSTALL_LABEL, L"SuccessInstallLabel" }, 
	{ WIXSTDBA_CONTROL_SUCCESS_UNINSTALL_LABEL, L"SuccessUninstallLabel" },
	{ WIXSTDBA_CONTROL_SUCCESS_REPAIR_LABEL, L"SuccessRepairLabel" }, 
...
LExit:
		...
		)
	{
		...
		// slimbyte update - AllowSameBundleVersionUpgrades - prevent same bundle version to be displayed twice in ARM
		...
		)
	{
		...
		{
			if (BOOTSTRAPPER_RELATED_OPERATION_NONE == m_Operation && 
				...
				BOOTSTRAPPER_RELATION_UPGRADE != m_command.relationType)
			{
				...
				if (m_fAllowSameBundleVersionUpgrades)
		...
		)
	{
		if (wzPackageId && *wzPackageId)
		{
			...
			// slimbyte update - Displaying more nice formated text
			WCHAR wzInfo[1024] = { };
			::StringCchPrintfW(wzInfo, countof(wzInfo), L"Downloading %s package...", wz);
		...
		)
	{
...
#ifdef DEBUG
		BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "WIXEXTBA: OnCacheAcquireProgress() - container/package: %ls, payload: %ls, progress: %I64u, total: %I64u, overall progress: %u%%", wzPackageOrContainerId, wzPayloadId, dw64Progress, dw64Total, dwOverallPercentage);
#endif
		...
		WCHAR wzInfo[1024] = { };
		::StringCchPrintfW(wzInfo, countof(wzInfo), L"Downloading %s package...  [ %u%% ]", wzPackageName, dwOverallPercentage);
		ThemeSetTextControl(m_pTheme, WIXSTDBA_CONTROL_OVERALL_PROGRESS_PACKAGE_TEXT, wzInfo);
	...
	{
#ifdef DEBUG
		BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "WIXEXTBA: OnExecuteMsiMessage() - package: %ls, message: %ls", wzPackageId, wzMessage);
#endif
...
#ifdef DEBUG
		BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "WIXEXTBA: OnProgress() - progress: %u%%, overall progress: %u%%", dwProgressPercentage, dwOverallProgressPercentage);
#endif
		...
		)
	{
		...
		if (wzPackageId && *wzPackageId)
		{
			...
			ThemeSetTextControl(m_pTheme, WIXSTDBA_CONTROL_EXECUTE_PROGRESS_PACKAGE_TEXT, wzInfo);
			ThemeSetTextControl(m_pTheme, WIXSTDBA_CONTROL_OVERALL_PROGRESS_PACKAGE_TEXT, wzInfo);
		}
		else
		{
...
#ifdef DEBUG
		BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "WIXEXTBA: OnExecuteProgress() - package: %ls, progress: %u%%, overall progress: %u%%", wzPackageId, dwProgressPercentage, dwOverallProgressPercentage);
#endif
...
LExit:
	...
	HRESULT InitializeData()
	{
		...
		if (SUCCEEDED(hr))
			m_pEngine->SetVariableString(WIXSTDBA_VARIABLE_BUNDLE_TEMP_PATH, sczModulePath);
...
LExit:
		...
		)
	{
		...
		hr = XmlSelectNodes(pixdManifest, L"/BootstrapperApplicationData/WixExtbaOverridableVariable", &pNodes);
		if (S_FALSE == hr)
		{
...
LExit:
		...
		)
	{
		...
		// slimbyte update - AllowSameBundleVersionUpgrades - prevent same bundle version to be displayed twice in ARM
		//static const LPCWSTR WIXSTDBA_VARIABLE_ALLOW_SAMEBUNDLEVERSIONUPGRADES = L"AllowSameBundleVersionUpgrades";
		hr = XmlGetAttributeNumber(pNode, L"AllowSameBundleVersionUpgrades", &dwBool);
		if (E_NOTFOUND == hr)
		{
			hr = S_OK;
		}
		...
		{
			m_fAllowSameBundleVersionUpgrades = 0 < dwBool;
		}
		BalExitOnFailure(hr, "Failed to get AllowSameBundleVersionUpgrades value.");


		// slimbyte update - DisallowAnotherInstanceRunning - prevent/allow same bundle to run at the same time 		
		hr = XmlGetAttributeNumber(pNode, L"DisallowAnotherInstanceRunning", &dwBool);
		if (E_NOTFOUND == hr)
		{
			hr = S_OK;
		}
		...
		{
			m_fDisallowAnotherInstanceRunning = 0 < dwBool;
		}
		BalExitOnFailure(hr, "Failed to get DisallowAnotherInstanceRunning value.");
...
LExit:
		...
		)
	{
		...
		m_fIsUninstall	= (m_plannedAction == BOOTSTRAPPER_ACTION_UNINSTALL); 
		if (m_plannedAction == BOOTSTRAPPER_ACTION_INSTALL) m_fOverallInstallationStarted = TRUE; 
		...
		)
	{
		...
		else // try to change the pages.
		{
			...
			if (dwOldPageId != dwNewPageId)
			{
				...
				else if (m_rgdwPageIds[WIXSTDBA_PAGE_PROGRESS] == dwNewPageId) // on the "PROGRESS" page, check if the restart or launch button should be enabled.
				{
					if ((BOOTSTRAPPER_ACTION_INSTALL == m_plannedAction && ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_INSTALL_HEADER)) ||
						(BOOTSTRAPPER_ACTION_UNINSTALL == m_plannedAction && ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_UNINSTALL_HEADER)) ||
						(BOOTSTRAPPER_ACTION_REPAIR == m_plannedAction && ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_REPAIR_HEADER)))
					{
						ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_HEADER, FALSE); 
						if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_LABEL))
							ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_LABEL, FALSE);
					}
					else
					{
						ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_HEADER, TRUE);
						if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_LABEL))
							ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_LABEL, TRUE);
					}

					if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_INSTALL_HEADER))
					{
						ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_INSTALL_HEADER, BOOTSTRAPPER_ACTION_INSTALL == m_plannedAction);
						if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_INSTALL_LABEL))
							ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_INSTALL_LABEL, BOOTSTRAPPER_ACTION_INSTALL == m_plannedAction);
					}

					if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_UNINSTALL_HEADER))
					{
						ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_UNINSTALL_HEADER, BOOTSTRAPPER_ACTION_UNINSTALL == m_plannedAction);
						if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_UNINSTALL_LABEL))
							ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_UNINSTALL_LABEL, BOOTSTRAPPER_ACTION_UNINSTALL == m_plannedAction);
					}

					if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_REPAIR_HEADER))
					{
						ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_REPAIR_HEADER, BOOTSTRAPPER_ACTION_REPAIR == m_plannedAction);
						if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_REPAIR_LABEL))
							ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_PROGRESS_REPAIR_LABEL, BOOTSTRAPPER_ACTION_REPAIR == m_plannedAction);
					}
				}
				else if (m_rgdwPageIds[WIXSTDBA_PAGE_SUCCESS] == dwNewPageId) // on the "Success" page, check if the restart or launch button should be enabled.
				{
						...
						(BOOTSTRAPPER_ACTION_REPAIR == m_plannedAction && ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_REPAIR_HEADER)))
					{
						...
						if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_LABEL))
							ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_LABEL, FALSE);
					}
					else
					{
						ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_HEADER, TRUE);
						if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_LABEL))
							ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_LABEL, TRUE);
					}
					...
					if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_INSTALL_HEADER))
					{
						...
						if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_INSTALL_LABEL))
							ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_INSTALL_LABEL, BOOTSTRAPPER_ACTION_INSTALL == m_plannedAction);
					}
					...
					if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_UNINSTALL_HEADER))
					{
						...
						if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_UNINSTALL_LABEL))
							ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_UNINSTALL_LABEL, BOOTSTRAPPER_ACTION_UNINSTALL == m_plannedAction);
					}
					...
					if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_REPAIR_HEADER))
					{
						...
						if (ThemeControlExists(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_REPAIR_LABEL))
							ThemeControlEnable(m_pTheme, WIXSTDBA_CONTROL_SUCCESS_REPAIR_LABEL, BOOTSTRAPPER_ACTION_REPAIR == m_plannedAction);
					}
				}
				else if (m_rgdwPageIds[WIXSTDBA_PAGE_FAILURE] == dwNewPageId) // on the "Failure" page, show error message and check if the restart button should be enabled.
				{
					...
					if (FAILED(m_hrFinal))
					{
						...
						// slimbyte update - Another installer instance is already running
						else if (E_WIXEXTBA_ANOTHER_INSTANCE_RUNNING == m_hrFinal)
						{
							StrAllocString(&sczText, L"Another instance of same installer is already running.", 0);
						}
						else
						{
...
LExit:
	...
	void OnClickInstallButton()
	{
		...
		if (BalStringVariableExists(WIXSTDBA_VARIABLE_XNDIAG_PREREQS))
		{
			if (MySQLDataBaseFilesFound())
			{
				int iUserChoice = ::MessageBoxW(m_hWnd, MSG_DATADIR_EXISTS, m_pTheme->sczCaption, MB_ICONEXCLAMATION | MB_YESNOCANCEL);    
				switch (iUserChoice)
				{
				case IDYES:
					m_pEngine->SetVariableNumeric(WIXSTDBA_VARIABLE_DONOTCLEANUPDATADIR, 1);
					bOkToContinue = true;
					break;
				case IDNO:
					bOkToContinue = true;
	...
	{
		// slimbyte update - LaunchAfterInstall feature - execute after install process on close button click
		BOOL fLaunchAfterInstallTargetExists = BalStringVariableExists(WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_TARGET_PATH);
		...
		{
			hr = BalGetStringVariable(WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_TARGET_PATH, &sczUnformattedLaunchTarget);
			BalExitOnFailure1(hr, "Failed to get launch after install target variable '%ls'.", WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_TARGET_PATH);

			hr = BalFormatString(sczUnformattedLaunchTarget, &sczLaunchTarget);
			BalExitOnFailure1(hr, "Failed to format launch after install target variable: %ls", sczUnformattedLaunchTarget);

			if (BalStringVariableExists(WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_ARGUMENTS))
			{
				hr = BalGetStringVariable(WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_ARGUMENTS, &sczUnformattedArguments);
				BalExitOnFailure1(hr, "Failed to get launch after install arguments '%ls'.", WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_ARGUMENTS);

				hr = BalFormatString(sczUnformattedArguments, &sczArguments);
				BalExitOnFailure1(hr, "Failed to format launch after install arguments variable: %ls", sczUnformattedArguments);
			}

			if (BalStringVariableExists(WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_HIDDEN))
			{
				nCmdShow = SW_HIDE;
			}		

			if (BalStringVariableExists(WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_WORK_FOLDER))
			{
				hr = BalGetStringVariable(WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_WORK_FOLDER, &sczLaunchFolder);
				BalExitOnFailure1(hr, "Failed to get launch after install working directory variable '%ls'.", WIXSTDBA_VARIABLE_LAUNCH_AFTERINSTALL_WORK_FOLDER);
			}			
			...
			if (m_fInstallSucceed && fLaunchAfterInstallTargetExists)
			{
				if (m_fRestartRequired)
				{
					hr = RegOpen(HKEY_CURRENT_USER, L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\RunOnce", KEY_ALL_ACCESS | KEY_WOW64_64KEY, &hkKey);
					BalExitOnFailure(hr, "Failed open RunOnce registry key");

					hr= PathEnsureQuoted(&sczLaunchTarget, FALSE);
					BalExitOnFailure(hr, "Failed to ensure quoted the LaunchTarget value.");
					if (sczArguments == NULL)
						sczRunOnceValue = sczLaunchTarget;
					else
					{
						hr = StrAllocFormatted(&sczRunOnceValue, L"%s %s", sczLaunchTarget, sczArguments);
						BalExitOnFailure(hr, "Failed to allocate and format the RunOnce value.");
					}

					hr = RegWriteString(hkKey, m_Bundle.sczName, sczRunOnceValue);
					BalExitOnFailure(hr, "Failed to write the RunOnce value into the registry.");
				}
				else
				{
					hr = ShelExec(sczLaunchTarget, sczArguments, L"open", sczLaunchFolder, nCmdShow, m_hWnd, NULL);
					BalExitOnFailure1(hr, "Failed to launch after install target: %ls", sczLaunchTarget);
				}
			}
		}
...
LExit:
		...
		)
	{
		if (BOOTSTRAPPER_DISPLAY_PASSIVE == m_command.display)
		{
			// slimbyte update - show a smaller dialog progress during passive commanline argument
			if (m_pTheme->fAutoResize)
			{
				SetWindowPos(m_hWnd,0,0,0,m_pTheme->nMinimumWidth,m_pTheme->nMinimumHeight,SWP_NOMOVE|SWP_NOZORDER|SWP_NOACTIVATE);
			}
	...
	HRESULT EvaluateConditions()
	{
		...
		if (hr == S_OK)
		{
			BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "WIXEXTBA: Looking if installer instance is already runing.");
			if (m_fAnotherInstanceRunning)
			{
				BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "WIXEXTBA: Instance of same installer is already running.");
				if (m_fDisallowAnotherInstanceRunning) {
					if (BOOTSTRAPPER_DISPLAY_NONE >= m_command.display)
					{
						BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "WIXEXTBA: Bundle is set to DisallowAnotherInstanceRunning, but this session is running quietly so, will allow it for now.");
					} 
					else
					{
						hr = E_WIXEXTBA_ANOTHER_INSTANCE_RUNNING;
						BalExitOnFailure(hr, "Instance of same installer is already running.");
					}
				}
			} else
				BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "WIXEXTBA: No other instance is already running.");
		}
...
LExit:
	...
	// slimbyte update - Remove "Action hh:mm:ss - " from the ActionData progress text 
	...
	// slimbyte update - Check for size of phisical memory
	void Populate_WiXPhysicalMemory_variable()
	{
		MEMORYSTATUSEX status;
		LONGLONG llPhysicalMemory;
		status.dwLength = sizeof(status);
		GlobalMemoryStatusEx(&status);
		llPhysicalMemory = (LONGLONG)(status.ullTotalPhys/(1024*1024));
		m_pEngine->SetVariableNumeric(WIXSTDBA_VARIABLE_PHYSICALMEMORY, llPhysicalMemory);
		return;
	}


	// slimbyte update - Check if any traces of MySQL Data Dir exists
	bool MySQLDataBaseFilesFound()
	{
		bool bRes = false;
		LPWSTR sczUnformattedText = NULL;
		LPWSTR wzInstallPath = NULL;
		LPWSTR wzTestFile1 = NULL;
		LPWSTR wzTestFile2 = NULL;
		LPWSTR wzTestFile3 = NULL;
		LPWSTR wzTestFile4 = NULL;
		LPWSTR wzTestFile5 = NULL;

		if (SUCCEEDED(BalGetStringVariable(WIXSTDBA_VARIABLE_INSTALL_FOLDER, &sczUnformattedText)))
		{
			BalFormatString(sczUnformattedText, &wzInstallPath);
			StrAllocFormatted(&wzTestFile1, L"%s\\MySQLData\\mysql\\db.frm", wzInstallPath);
			StrAllocFormatted(&wzTestFile2, L"%s\\MySQLData\\mysql\\columns_priv.frm", wzInstallPath);
			StrAllocFormatted(&wzTestFile3, L"%s\\MySQLData\\mysql\\tables_priv.frm", wzInstallPath);
			StrAllocFormatted(&wzTestFile4, L"%s\\MySQLData\\mysql\\func.frm", wzInstallPath);
			StrAllocFormatted(&wzTestFile5, L"%s\\MySQLData\\mysql\\user.frm", wzInstallPath);

			bRes = FileExistsEx(wzTestFile1, NULL) && FileExistsEx(wzTestFile2, NULL) && FileExistsEx(wzTestFile3, NULL) && 
				FileExistsEx(wzTestFile4, NULL) && FileExistsEx(wzTestFile5, NULL);
		}
...
public:
		...
		) : CBalBaseBootstrapperApplication(pEngine, pCommand, 3, 3000)
	{
		...
		// slimbyte update - DisallowAnotherInstanceRunning - prevent/allow same bundle to run at the same time 
		LPWSTR sczBundleMutex = NULL;
		BOOL   bMutexStringSucceded = FALSE;

		if (BalStringVariableExists(L"WixBundleTag"))
		{
			bMutexStringSucceded = SUCCEEDED(BalGetStringVariable(L"WixBundleTag", &sczBundleMutex)) && (StrCmpCW(sczBundleMutex, L"") != 0);
			if (bMutexStringSucceded)
				BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "WIXEXTBA: Using WixBundleTag for Mutex: %ls.", sczBundleMutex);
		}
		if (!bMutexStringSucceded)
		{
			bMutexStringSucceded = SUCCEEDED(BalGetStringVariable(L"WixBundleProviderKey", &sczBundleMutex));
			if (bMutexStringSucceded)
				BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "WIXEXTBA: Using WixBundleProviderKey for Mutex: %ls.", sczBundleMutex);
		}

		// Initializing bootstrapper mutex to detct if another instance is running 
		if (bMutexStringSucceded)
		{
			DWORD  m_dwLastError;
			m_hInstallerMutex = CreateMutexW(NULL, FALSE, sczBundleMutex /*TEXT("Global\\{05CA3573-B449-4e0b-83F5-7FD612E378E9}")*/); //do early
			m_dwLastError = GetLastError(); //save for use later...
			m_fAnotherInstanceRunning = (ERROR_ALREADY_EXISTS == m_dwLastError);
			if (m_hInstallerMutex == NULL)
				BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "WIXEXTBA: Failed to initialize mutex.");
		} else
			BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "WIXEXTBA: Failed to get WixBundleProviderKey for mutex.");
		...
		m_fAllowSameBundleVersionUpgrades = FALSE;
		m_fDisallowAnotherInstanceRunning = FALSE;
		...
		Populate_WiXPhysicalMemory_variable();
	...
	~CWixStandardBootstrapperApplication()
	{
		...
		if (m_hInstallerMutex)
		{
			CloseHandle(m_hInstallerMutex);
			m_hInstallerMutex = NULL;
		}
...
private:
	...
	BOOL m_fAllowSameBundleVersionUpgrades;
	BOOL m_fDisallowAnotherInstanceRunning;
	...
	HANDLE m_hInstallerMutex;
	BOOL m_fAnotherInstanceRunning;
