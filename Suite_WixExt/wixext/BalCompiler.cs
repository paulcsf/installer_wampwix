...
namespace Microsoft.Tools.WindowsInstallerXml.Extensions
{
    ...
    public sealed class BalCompiler : CompilerExtension
    {
                    ...
                    else
                    {
                        switch (attribute.LocalName)
                        {
                            case "Overridable":
                                if (YesNoType.Yes == this.Core.GetAttributeYesNoValue(sourceLineNumbers, attribute))
                                {
                                    Row row = this.Core.CreateRow(sourceLineNumbers, "WixExtbaOverridableVariable");
                                    row[0] = variableName;
                                }
                                break;
                            default:
        ...
        private void ParseWixExtendedBootstrapperApplicationElement(XmlNode node)
        {
            ...
            string launchAfterInstallTarget = null;
            string launchAfterInstallArguments = null;
            YesNoType launchAfterInstallHidden = YesNoType.NotSet;
            string launchAfterInstallWorkingDir = null;
			string installationLogsDir = null;
            ...
            YesNoType allowSameBundleVersionUpgrades = YesNoType.NotSet;
            YesNoType disallowAnotherInstanceRunning = YesNoType.NotSet;
                ...
                if (0 == attrib.NamespaceURI.Length || attrib.NamespaceURI == this.schema.TargetNamespace)
                {
                    switch (attrib.LocalName)
                    {
                        ...
                        case "LaunchWorkingFolder":
                            ...
                            break;
                        case "LaunchAfterInstallTarget":
                            launchAfterInstallTarget = this.Core.GetAttributeValue(sourceLineNumbers, attrib, false);
                            break;
                        case "LaunchAfterInstallArguments":
                            launchAfterInstallArguments = this.Core.GetAttributeValue(sourceLineNumbers, attrib, false);
                            break;
                        case "LaunchAfterInstallHidden":
                            launchAfterInstallHidden = this.Core.GetAttributeYesNoValue(sourceLineNumbers, attrib);
                            break;
                        case "LaunchAfterInstallWorkingFolder":
                            launchAfterInstallWorkingDir = this.Core.GetAttributeValue(sourceLineNumbers, attrib, false);
                            break;
                        case "LicenseFile":
                        ...
                        case "ShowFilesInUse":
                            ...
                            break;
                        case "AllowSameBundleVersionUpgrades":
                            allowSameBundleVersionUpgrades = this.Core.GetAttributeYesNoValue(sourceLineNumbers, attrib);
                            break;
                        case "DisallowAnotherInstanceRunning":
                            disallowAnotherInstanceRunning = this.Core.GetAttributeYesNoValue(sourceLineNumbers, attrib);
                            break;
                        case "InstallationLogsFolder":
                            installationLogsDir = this.Core.GetAttributeValue(sourceLineNumbers, attrib, false);
                            break;
                        default:
                            this.Core.UnexpectedAttribute(sourceLineNumbers, attrib);
                            break;
            ...
            if (!this.Core.EncounteredError)
            {
                ...
                if (!String.IsNullOrEmpty(launchAfterInstallTarget))
                {
                    this.Core.CreateVariableRow(sourceLineNumbers, "LaunchAfterInstallTarget", launchAfterInstallTarget, "string", false, false);
                }

                if (!String.IsNullOrEmpty(launchAfterInstallArguments))
                {
                    this.Core.CreateVariableRow(sourceLineNumbers, "LaunchAfterInstallArguments", launchAfterInstallArguments, "string", false, false);
                }

                if (YesNoType.Yes == launchAfterInstallHidden)
                {
                    this.Core.CreateVariableRow(sourceLineNumbers, "LaunchAfterInstallHidden", "yes", "string", false, false);
                }
				
                if (!String.IsNullOrEmpty(launchAfterInstallWorkingDir))
                {
                    this.Core.CreateVariableRow(sourceLineNumbers, "LaunchAfterInstallWorkingFolder", launchAfterInstallWorkingDir, "string", false, false);
                }

				
				if (!String.IsNullOrEmpty(installationLogsDir))
                {
                    this.Core.CreateVariableRow(sourceLineNumbers, "InstallationLogsFolder", installationLogsDir, "string", false, false);
                }
                ...
                if (YesNoType.Yes == suppressOptionsUI || YesNoType.Yes == suppressDowngradeFailure || YesNoType.Yes == suppressRepair || YesNoType.Yes == showVersion || YesNoType.Yes == supportCacheOnly || YesNoType.Yes == showFilesInUse || YesNoType.Yes == allowSameBundleVersionUpgrades || YesNoType.Yes == disallowAnotherInstanceRunning)
                {
                    ...
                    if (YesNoType.Yes == allowSameBundleVersionUpgrades)
                    {
                        row[6] = 1;
                    }
					
                    if (YesNoType.Yes == disallowAnotherInstanceRunning)
                    {
                        row[7] = 1;
                    }
                }
